...
export const goAddShoppingItem = (item, userId) => {
    return async (dispatch) => {
        try {
            const { data: product } = await axios.post('/api/shoppingItem', { itemId: item.id, userId, qty: 1 });
...
export const goIncrementShoppingItem = (item, userId, qty) => {
    return async (dispatch) => {
        try {
            const { data: product } = await axios.put(`/api/shoppingItem/${item.id}`, userId, qty)
        } catch (err) {
            console.log(err)
        }
    }
